//DATOS DE LA BASE DE DATOS
let productosBD=[
    {
        nombre:'Adidas Forum 84 Low ADV', //Forum84
        precio: 499990,
        cantidad:3,
        foto:['https://firebasestorage.googleapis.com/v0/b/livtiendatcc.appspot.com/o/Forum84.png?alt=media&token=52c7ae41-64f1-4e00-80b5-4f7cd6855e15','https://firebasestorage.googleapis.com/v0/b/livtiendatcc.appspot.com/o/forum84dos.webp?alt=media&token=15181af1-d067-4d5b-b0f9-fd125c11907e'],
        Descripcion:'Adidas se suma a la estética retro con sus Adidas Forum 84 Low ADV, uno de sus modelos más populares para este año, con colores blanco y crudo.'
    },

    {
        nombre:'Adidas Ozelia', //Ozelia
        precio:459990,
        cantidad:1,
        foto:'https://firebasestorage.googleapis.com/v0/b/livtiendatcc.appspot.com/o/Ozelia.jpg?alt=media&token=f6e04f95-02c0-484c-9dd2-630d67895e91',
        Descripcion:'Una mezcla casi imposible entre lo noventero y lo futurista. equipado con la calidad : amortiguación Adiprene, esfuerzos de piel y silueta envolvente. Aunque están disponibles en un total de once colores, las "Off White" son de las más populares.',
    },
    {
        nombre:'Adidas PW Hu NMD', //AdPWHu
        precio:890990,
        cantidad:2,
        foto:'https://firebasestorage.googleapis.com/v0/b/livtiendatcc.appspot.com/o/AdPWHu.png?alt=media&token=37fb2369-d08f-4f13-9381-5860b1aac763',
        Descripcion:'Una mezcla única de colores fabricada con materiales ecológicos y de forma sostenible. Cuenta con un sistema de cordones especial que no presiona sobre el empeine y garantiza sujeción y confort para el pie.',
    },
    {
        nombre:'Adidas Yeezy Boost 700 MNVN', // Yeezy
        precio:2500000,
        cantidad:1,
        foto:'https://firebasestorage.googleapis.com/v0/b/livtiendatcc.appspot.com/o/Yeezy.jpg?alt=media&token=7bb2eb3d-4bb4-454e-b6c6-71f85877e638',
        Descripcion:'Apuestan por la modernidad con un diseño único, de esos que se identifican con un único vistazo y desde lejos. Su forma almendrada y la robusta suela es lo que le confiere su personalidad llamativa.',
    },
    {
        nombre:'Air Jordan 6 Low CNY', //23low
        precio:666845,
        cantidad:3,
        foto:'https://firebasestorage.googleapis.com/v0/b/livtiendatcc.appspot.com/o/23low.jpg?alt=media&token=3672074a-3874-4659-ae4e-3bb6a08c2de5',
        Descripcion:'Un diseño muy retro en base cruda con detalles en colores suaves, un bronce metalizado y un azul traslúcido, que pretenden hacer homenaje al año del tigre.',
    },
    {
        nombre:'Converse Star Player 76 Premium Canvas', //Star76
        precio:320000,
        cantidad:3,
        foto:'https://firebasestorage.googleapis.com/v0/b/livtiendatcc.appspot.com/o/Star76.jpg?alt=media&token=79e2f38a-5246-4872-b360-a4d62c8546a8',
        Descripcion:'Inspiradas en las deportivas de baloncesto retro, con una comodidad y una estética adaptadas a los tiempos actuales. De corte bajo, transpirables y con un sistema de amortiguación que agrega comodidad en cada pisada, perfectas para el lifestyle.',
    },
    {
        nombre:'Converse x Todd Snyder Chuck 70 HI', // Chuck70
        precio:47900,
        cantidad:4,
        foto:'https://firebasestorage.googleapis.com/v0/b/livtiendatcc.appspot.com/o/Chuck70.png?alt=media&token=4d1ed8b6-9011-456c-a76b-a5fe935b7a8c',
        Descripcion:'. Elegantes, clásicas y de inspiración militar, un calzado cuidado hasta el más mínimo detalle que se convertirá en tu favorito.',
    },
    {
        nombre:'Lacoste L001', //L001
        precio:31700,
        cantidad:2,
        foto:'https://firebasestorage.googleapis.com/v0/b/livtiendatcc.appspot.com/o/L001.jpg?alt=media&token=113a1bec-0ff5-4396-a87b-0e4796bb6536',
        Descripcion:'Estas Lacoste lucen minimalistas, pero sin renunciar a la personalidad y distinción. Salpicadas por detalles de color sobre lienzo blanco y suela gruesa.',
    },
    {
        nombre:'Le Coq Sportif Terra Joyeuse', //Joyeuse
        precio:513000,
        cantidad:3,
        foto:'https://firebasestorage.googleapis.com/v0/b/livtiendatcc.appspot.com/o/Joyeuse.jpg?alt=media&token=65e667ae-d021-4117-bee8-52b715bd3eed',
        Descripcion:'Su diseño se debe a una colaboración solidaria entre los restaurantes Café Joyeux y la marca Le Coq Sportif. El objetivo consiste en recaudar fondos para fomentar la contratación de personas con Síndrome de Down y otras condiciones cognitivas, como el autismo.',
    },
    {
        nombre:'New Balance 530', //New530
        precio:390000,
        cantidad:5,
        foto:'https://firebasestorage.googleapis.com/v0/b/livtiendatcc.appspot.com/o/New530.jpg?alt=media&token=ea283615-66d6-4d2a-b2fa-4cd4f64c01f1',
        Descripcion:'viene acompañada de una suela ABZORB, un empeine de malla transpirable y un aire retro que las postula entre las más calzadas del año.',
    },
    {
        nombre:'New Balance 990v4', //New990
        precio:419000,
        cantidad:2,
        foto:'https://firebasestorage.googleapis.com/v0/b/livtiendatcc.appspot.com/o/New990.png?alt=media&token=ec3c63d0-7a62-4327-83f6-0a1323174d6d',
        Descripcion:'Un diseño que se viene reinventando desde que viese la luz por primera vez en el año 1978. Lo que las hace únicas es su comodidad y la amortiguación, gracias a un sistema altamente eficiente que combina una espuma ligera en su entresuela con un borde de poliuretano para mayor durabilidad.',
    },
    {
        nombre:'New Balance Conversations Amongst Us', //NewUs
        precio:598000,
        cantidad:2,
        foto:'https://firebasestorage.googleapis.com/v0/b/livtiendatcc.appspot.com/o/NewUs.png?alt=media&token=abdad6ce-fbee-43f8-8be2-5dd6cf6f88ca',
        Descripcion:'Una reinterpretación del modelo 2002 en colores llamativos que se mueven dentro de las tonalidades anaranjadas y amarillas, sobre un fondo crudo. El diseño de este modelo ha corrido a cargo del director creativo Joe Freshgoods.1',
    },
    {
        nombre:'New Balance Tom Knox 440', //New440
        precio:320000,
        cantidad:1,
        foto:'https://firebasestorage.googleapis.com/v0/b/livtiendatcc.appspot.com/o/New440.jpg?alt=media&token=06dba867-c7b4-48da-a47b-a94a2ce736c2',
        Descripcion:'esencialmente urbano, en un total black con empeine de ante y malla para una mayor comodidad. Está reforzado para evitar el desgaste y aumentar así su resistencia y durabilidad. Un modelo sencillo con un punto sofisticado pensado para el street style.',
    },
    {
        nombre:'New Balance XC-72', //NewXC
        precio:557000,
        cantidad:3,
        foto:'https://firebasestorage.googleapis.com/v0/b/livtiendatcc.appspot.com/o/NewXC.jpg?alt=media&token=33f93429-aed7-42d5-a58b-04fce4506136',
        Descripcion:'Caracterizadas por la extensión de la suela hacia el lateral de la puntera y un revestimiento de goma en el talón, con la gran N inconfundible de New Balance.',
    },
    {
        nombre:'Nike Air Jordan 1 Mid', //NikeJordan
        precio:533000,
        cantidad:3,
        foto:'https://firebasestorage.googleapis.com/v0/b/livtiendatcc.appspot.com/o/NikeJordan.jpg?alt=media&token=e2b1dd11-1f8e-4097-9303-3ac096f94520',
        Descripcion:'',
    },
    {
        nombre:'Nike Air Max Terrascape 90', //Nike90
        precio:770000,
        cantidad:4,
        foto:'https://firebasestorage.googleapis.com/v0/b/livtiendatcc.appspot.com/o/Nike90.jpg?alt=media&token=5dff2803-5f5f-4114-b38a-ccb4a4ba2fe5',
        Descripcion:'Cómodas, duraderas y con buena amortiguación, su diseño también es ligero y transpirable. Luce un estilo retro propio de los años 90 con colores tierra, crudos y un azul oscuro en cordones y logotipo.',
    },
    {
        nombre:'Nike Blazer Low x Off-White', // NikeLow
        precio:550000,
        cantidad:2,
        foto:'https://firebasestorage.googleapis.com/v0/b/livtiendatcc.appspot.com/o/NikeLow.jpg?alt=media&token=8b0d3f24-0891-406f-8bfc-fe696d50bb0c',
        Descripcion:'Son un diseño transgresor, único y rompedor, con una pieza que sobresale en el talón, detalles coloridos y aberturas.',
    },
    {
        nombre:'Nike Blazer Mid 77', //Nike77
        precio:309000,
        cantidad:4,
        foto:'https://firebasestorage.googleapis.com/v0/b/livtiendatcc.appspot.com/o/Nike77.jpg?alt=media&token=1a189438-e025-4bdc-8819-3316ce0d25a0',
        Descripcion:'Un clásico que no ha perdido ni un ápice de autenticidad y aire vintage. La colección y variación de colores ofrece diseños para todos los gustos, pero las más emblemáticas son las blancas con el logo en negro.',
    },
    {
        nombre:'Nike Dunk Low', //NikeDunk
        precio:480000,
        cantidad:2,
        foto:'https://firebasestorage.googleapis.com/v0/b/livtiendatcc.appspot.com/o/NikeDunk.jpg?alt=media&token=b53d83d5-dc03-408e-b193-9d15805b2dbf',
        Descripcion:' La versión más deseada es la Nike Dunk Low Retro, que se creó para la cancha de baloncesto y ahora despuntan entre las más buscadas para el asfalto. Un icono de los años 80 que no hace sino ganar cada vez más fama.',
    },
    {
        nombre:'Nike Go FlyEase', //NikeGo
        precio:458000,
        cantidad:2,
        foto:'https://firebasestorage.googleapis.com/v0/b/livtiendatcc.appspot.com/o/NikeGo.png?alt=media&token=e3c6f1d0-b465-48b4-9c69-6c29faf45a36',
        Descripcion:'Han sido un bombazo gracias a su sistema de descalzado sin manos; las Nike Go FlyEase son unas zapatillas que te podrás poner y quitar en apenas un segundo, sin siquiera tener que agacharte. Han sido fabricadas con una tecnología que permite abrir la parte del talón para introducir o sacar el pie sin necesidad de usar las manos.',
    },
    {
        nombre:'Nike x Drake Hot Step Air Terra', //NikeDrake
        precio:2920000,
        cantidad:3,
        foto:'https://firebasestorage.googleapis.com/v0/b/livtiendatcc.appspot.com/o/NikeDrake.jpg?alt=media&token=8f8bf662-f5d2-415a-b92c-034c498356f6',
        Descripcion:'Lo más destacado de su diseño es sin duda su detalle de perforaciones, que adorna tanto laterales como parte del frontal del calzado. Están fabricadas en cuero blanco y son una colaboración con el rapero Drake.',
    },
    {
        nombre:'Paris High Top Full Destroyed de Balenciaga', //Paris
        precio:6200000,
        cantidad:1,
        foto:'https://firebasestorage.googleapis.com/v0/b/livtiendatcc.appspot.com/o/Paris.jpg?alt=media&token=5d9ba665-da69-4fad-a8c2-e409cc503a07',
        Descripcion:'Estas zapatillas se han hecho virales por su afán de promover una moda más sostenible. Con el fin de concienciar sobre la durabilidad de las prendas, se presentan en un algodón totalmente desgastado, desgarrada y la suela de caucho manchada. Llevan el logotipo de Balenciaga en grafiti y son ya un fenómeno en la red.',
    },
    {
        nombre:'Prada x Adidas Forum Hi', //PradaAdd
        precio:3600000,
        cantidad:2,
        foto:'https://firebasestorage.googleapis.com/v0/b/livtiendatcc.appspot.com/o/PradaAdd.jpg?alt=media&token=203adad2-b528-43d0-94ae-54275fcdd3b9  ',
        Descripcion:'la reinterpretación al estilo Prada de las Adidas Forum, confeccionadas con los materiales propios de la firma de lujo. Posiblemente lo primero que llama la atención al verlas es su particular compartimento extraíble anudado al tobillo.',
    },
    {
        nombre:'Premiata Steven 5247', //Premiata
        precio:1342000,
        cantidad:1,
        foto:'https://firebasestorage.googleapis.com/v0/b/livtiendatcc.appspot.com/o/Premiata.jpg?alt=media&token=66a80ed7-6f37-410a-97bc-5f3589c54064',
        Descripcion:'Las zapatillas de Premiata se fabrican con los mejores tejidos y pieles italianas. Sus tratamientos y lavados propios son inconfundibles, con su suela estampada en relieve. Las Steven 5247 son tan originales como sofisticadas.',
    },
    {
        nombre:'Re Ground Zèta x Nespresso', //Nespresso
        precio:662000,
        cantidad:1,
        foto:'https://firebasestorage.googleapis.com/v0/b/livtiendatcc.appspot.com/o/Nespresso.jpg?alt=media&token=0241594e-62d8-4de3-8b2d-d5c083070578',
        Descripcion:'zapatillas ecológicas producidas con posos de café obtenidos del reciclaje de las cápsulas de Nespresso. Este café se ha integrado en los empeines y en las suelas, mientras que el resto de la zapatilla está fabricada en un 80 % por materiales reciclados y sostenibles.',
    },
    {
        nombre:'Reebok Club C Revenge', //Revenge
        precio:600000,
        cantidad:2,
        foto:'https://firebasestorage.googleapis.com/v0/b/livtiendatcc.appspot.com/o/Revenge.jpg?alt=media&token=39f12db3-0ab3-4c3d-97b8-74d9774ca876',
        Descripcion:'Estas Reebok se inspiran en las pistas de tenis de los años 80, con diseño minimalista y rayas laterales en contraste.',
    },
    {
        nombre:'Reebok Premier Road Plus VI X JUL', //ReJul
        precio:537000,
        cantidad:3,
        foto:'https://firebasestorage.googleapis.com/v0/b/livtiendatcc.appspot.com/o/ReJul.jpg?alt=media&token=8ee22ab4-b4ba-4e20-ab21-36b1386bb572',
        Descripcion:'Estética atrevida, colores agresivos y formas duras. En 2009 se lanzaron las Reebok Premier Road Plus y, una década más tarde, han sido reinventadas con la participación del rapero francés Jul en el proyecto.',
    },
    {
        nombre:'Reebok Pump Omni Zone II', //ReebokII
        precio:370000,
        cantidad:4,
        foto:'https://firebasestorage.googleapis.com/v0/b/livtiendatcc.appspot.com/o/ReebokII.png?alt=media&token=d3b45f41-f962-4f17-ad8d-eaa1b244a4bf',
        Descripcion:'Una vuelta a la estética retro del baloncesto de los años 90  con su singular estilo, llamativo como él solo gracias a su silueta y potentes colores que recuerdan a los de una llama de fuego. ',
    },
    {
        nombre:'Vulcan de Timpers', //Vulcan
        precio:256000,
        cantidad:1,
        foto:'https://firebasestorage.googleapis.com/v0/b/livtiendatcc.appspot.com/o/Vulcan.jpg?alt=media&token=009d19f7-af29-4164-af88-295b8759cead',
        Descripcion:'Un modelo de negocio que fomenta la inclusión, usando materiales reciclados en la fabricación. Entre sus muchos modelos, las Vulcan son de las más vendidas y están disponibles en gran variedad de diseños y combinaciones de colores.',
    },
    {
        nombre:'Vans x Crayola',  //Crayola
        precio:230000,
        cantidad:3,
        foto:'https://firebasestorage.googleapis.com/v0/b/livtiendatcc.appspot.com/o/Crayola.png?alt=media&token=1e24b650-4e79-4741-b0ed-13fa6596b084',
        Descripcion:'Una de las colaboraciones más alocadas y divertidas, las zapatillas Authentic nacen de la unión entre las marcas Vans y Crayola. Completamente repletas de dibujos y caricaturas, cuentan con toques de colores saturados que dan vida al estilo más clásico de las deportivas de Vans.',
    }


]

console.log(productosBD[5].nombre)


//Recorriendo un array(arreglo) esculcar

productosBD.forEach(function(producto){
    console.log(producto.nombre)
    console.log(producto.foto)

})

// PASOS PARA PINTAR (HACER RENDER) DE ETIQUETAS HTML DESDE JS --- TRAVERSING

//1. cREAR UNA REFERENCIA EN MEMORIA (VARIABLE) QUE ALMACENE LA ETIQUETA SONBRE LA CUAL VOY A PINTAR (ETIQUETA PADRE O RAIZ)

let fila=document.getElementById('fila')

// 2. recorro el arreglo de datos y aplico traversing 
productosBD.forEach(function(producto){
    //2.1 creando una columana desde js
    let columna=document.createElement('div')
    columna.classList.add('col')

    //2.2 creando tarjeta
    let tarjeta=document.createElement('div')
    tarjeta.classList.add('card','h1-100','text-center')

    //2.3 creando la foto
    let fotoproducto=document.createElement('img')
    fotoproducto.classList.add('img-fluid','w-100','h-100')
    fotoproducto.src=producto.foto
    //2.4 creando el nombre
    let nombreProducto=document.createElement('h3')
    nombreProducto.textContent=producto.nombre
    //2.5 Creando precio
    let precioProducto=document.createElement('h2')
    precioProducto.classList.add('fw-bold')
    precioProducto.textContent='$'+producto.precio+'COP'

    //2.6 Detectando eventos
    columna.addEventListener('mouseover',function(){
    fotoproducto.src=producto.foto[1]  
    })
    columna.addEventListener('mouseleave',function(){
        fotoproducto.src=producto.foto
    })
    //3. defino los parder e hijos (Jerarquias)
    tarjeta.appendChild(fotoproducto)
    tarjeta.appendChild(nombreProducto)
    tarjeta.appendChild(precioProducto)
    columna.appendChild(tarjeta)

    fila.appendChild(columna)

})

function saludar (nombre){
    console.log('hola'+nombre)

}
let nombre='juan';
saludar('catalina');



function sumar(n1,n2){
return(n1+n2)
}
sumar(2,5);
console.log(sumar(2,5))